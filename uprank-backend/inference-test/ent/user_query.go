...
func (uq *UserQuery) QueryJob() *JobQuery {
...
func (uq *UserQuery) QueryUpworkjob() *UpworkJobQuery {
	query := (&UpworkJobClient{config: uq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := uq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (uq *UserQuery) Clone() *UserQuery {
	...
	return &UserQuery{
		...
		withJob:       uq.withJob.Clone(),
		withUpworkjob: uq.withUpworkjob.Clone(),
...
func (uq *UserQuery) WithJob(opts ...func(*JobQuery)) *UserQuery {
	query := (&JobClient{config: uq.config}).Query()
...
func (uq *UserQuery) WithUpworkjob(opts ...func(*UpworkJobQuery)) *UserQuery {
	query := (&UpworkJobClient{config: uq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	uq.withUpworkjob = query
	return uq
}
...
func (uq *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := uq.withJob; query != nil {
		if err := uq.loadJob(ctx, query, nodes,
			func(n *User) { n.Edges.Job = []*Job{} },
			func(n *User, e *Job) { n.Edges.Job = append(n.Edges.Job, e) }); err != nil {
			return nil, err
		}
	}
	if query := uq.withUpworkjob; query != nil {
		if err := uq.loadUpworkjob(ctx, query, nodes,
			func(n *User) { n.Edges.Upworkjob = []*UpworkJob{} },
			func(n *User, e *UpworkJob) { n.Edges.Upworkjob = append(n.Edges.Upworkjob, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (uq *UserQuery) loadJob(ctx context.Context, query *JobQuery, nodes []*User, init func(*User), assign func(*User, *Job)) error {
	...
	query.Where(predicate.Job(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.JobColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.user_job
		if fk == nil {
			return fmt.Errorf(`foreign-key "user_job" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "user_job" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
}
func (uq *UserQuery) loadUpworkjob(ctx context.Context, query *UpworkJobQuery, nodes []*User, init func(*User), assign func(*User, *UpworkJob)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(user.UpworkjobTable)
		s.Join(joinT).On(s.C(upworkjob.FieldID), joinT.C(user.UpworkjobPrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(user.UpworkjobPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(user.UpworkjobPrimaryKey[0]))
	...
	for _, n := range neighbors {
		nodes, ok := nids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected "upworkjob" node returned %v`, n.ID)
		}
		for kn := range nodes {
			assign(kn, n)
		}
